...
class OrdemProducao  {
	 ...
	 * @param Formata 
	 */
    public function OrdemProducao($ConexaoSQL){
	...
	*/
	public function pegaOrdemProducao($produto = "", $pedido = "", $dataIni = "", $dataFim = "", $idOrdem = "", $limite = ""){
		...
		if(!empty($dataIni))
			$busca .= " AND ordem_producao.data_cad >= '".Formata::date2banco($dataIni)."' ";
			
		if(!empty($dataFim))
			$busca .= " AND ordem_producao.data_cad <= '".Formata::date2banco($dataFim)."' ";
	...
	*/
	public function salvarOrdemProducao($id = "", $produto = "", $pedido = "", $qtd = "", $descricao = "", $data = "", $status = ""){
		if(empty($id)){
			$this->ConexaoSQL->insertQuery("INSERT INTO ordem_producao (id_produtos, id_pedido, qtd, descricao, id_status_ordem, data_cad) VALUES('".$produto."', '".$pedido."','".$qtd."','".$descricao."', '1', '".Formata::date2banco($data)."')");
		}else{
			$this->ConexaoSQL->updateQuery("UPDATE ordem_producao SET id_produtos = '".$produto."', descricao = '".$descricao."', id_pedido = '".$pedido."', qtd = '".$qtd."', id_status_ordem= '1', data_cad = '".Formata::date2banco($data)."' WHERE id = '".$id."'");
		}
	}
