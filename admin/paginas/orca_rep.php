...
if($acao=="alt"){
	$id=$_GET["id"];
	$res=mysql_fetch_array(mysql_query("SELECT * FROM cad_orca_rep WHERE id='$id'"));
}else if($acao=="alterar"){
	$id=$_POST["id"];
	$valor=valor2banco($valor);
	$sql=mysql_query("UPDATE cad_orca_rep SET numero='$numero',nome='$nome',n_orc='$n_orc',descr='$desc',contato='$contato',tel='$tel',valor='$valor',obs='$obs',status='$status' WHERE id='$id'");
	print "<script>window.location='index.php?url=orca_rep.php'</script>";
}else if($acao=="incluir"){
	$valor=valor2banco($valor);
	$sql=mysql_query("INSERT INTO cad_orca_rep (numero,nome,n_orc,descr,contato,tel,valor,obs,status,data_cad,usuario) VALUES('$numero','$nome','$n_orc','$desc','$contato','$tel','$valor','$obs','$status','$hj','$usua')") or die(mysql_error());
	print "<script>window.location='index.php?url=orca_rep.php'</script>";
	//exit;
}else if($acao=="exc"){
	$id=$_GET["id"];
	$sql=mysql_query("DELETE FROM cad_orca_rep WHERE id='$id'");
	print "<script>window.location='index.php?url=orca_rep.php'</script>";
	//exit;
}
?>
...
<h1>Orçamentos de reparo</h1>
<div align="center">
  <table width="417" border="0" align="left" cellpadding="0" cellspacing="0">
    <tr>
      <th scope="col"><a href="index.php?url=orca_rep.php&acao=inc">Incluir Orçamento</a></th>
    </tr>
  </table>
  <br>
</div>
<div align="left"></div>
<? 
$sql=mysql_query("SELECT * FROM cad_orca_rep WHERE usuario='$usua' ORDER By id DESC");
...
&nbsp;&nbsp;&nbsp;&nbsp;<strong>Status:</strong><?= $st; ?></td>
        ...
        <tr align="center">
          <td colspan="2"><input type="button" name="Button" value="Alterar" onClick="window.location='index.php?url=orca_rep.php&acao=alt&id=<?= $res["id"]; ?>'">
          &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
          <input type="button" name="Button2" value="Excluir" onClick="if(confirm('Deseja mesmo excluir?')){ window.location='index.php?url=orca_rep.php&acao=exc&id=<?= $res["id"]; ?>'; }"></td>
        </tr>
    </table></th>
  </tr>
</table>
...
<form name="form1" method="post" action="">
  <table width="350" border="0" cellpadding="0" cellspacing="0">
    <tr>
      ...
      <th width="199" align="left" scope="col"><? $vp->campo("text","numero",$res["numero"]); ?></th>
    </tr>
    <tr>
