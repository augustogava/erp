...
function addPop_openLoading(width) {
	...
	popup.style.top = (windowS.getScrollTop()+ (windowS.getHeight()/2)) + 'px';
	popup.style.margin = '-'+ (popup.offsetHeight/2) +'px 0 0 -'+ (width/2) + 'px';
	
	jQuery(".modal").show();
	
	if( $('main-body') != undefined )
		$('main-body').onclick = function(){ return false; }
}
...
function addPop_open(width) {
	...
	popup.style.width = (width) +"px";
	popup.style.left = (windowS.getWidth()/2) + 'px';
	popup.style.top = (windowS.getScrollTop()+ (windowS.getHeight()/2)) - (windowS.getHeight()/3) + 'px';
	popup.style.margin = '-'+ (popup.offsetHeight/2) +'px 0 0 -'+ (width/2) + 'px';
...
function addPop_close() {
	...
	if( $('main-body') != undefined )
		$('main-body').onclick = function(){ return true; }
...
function PegarCampos(){
	...
	inputs = inputs.concat(Campo2Array(document.getElementsByTagName("select")));
...
function abrirPopVisu(campoatual, tabela){
	...
	window.open('visualizarpopup.php?campoatual=' + campoatual + '&tabela=' + tabela + '&id=' + $(campoatual).value , 'Busca', "height=600, width=550, scrollbars=yes, resizable=yes")
...

{
    ...
    doAjax(url + "?login=" + login + "&acao=" + acao + "&senha=" + senha, 'retorno_login', 2, 'lendo');
...
function abrirRelatorioClientes(){
	...
	window.open('relatorios.php?cadastro=clientes&filtro1=' + filtro1 + '&filtro2=' + filtro2 + '&filtro3=' + filtro3 + '&filtro4=' + filtro4, "janelaRelatorios", "height=600, width=800, scrollbars=yes, resizable=yes");
...
function abrirRelatorioCurvaABC(){
	...
	window.open('relatorios.php?cadastro=curvaabc&filtro1=' + filtro1 + '&filtro2=' + filtro2 + '&filtro3=' + filtro3, "janelaRelatorios", "height=600, width=800, scrollbars=yes, resizable=yes");
...
function abrirRelatorioRepresentantes(){
	...
	window.open('relatorios.php?cadastro=representantes&filtro1=' + filtro1 + '&filtro2=' + filtro2 + '&filtro3=' + filtro3, "janelaRelatorios", "height=600, width=800, scrollbars=yes, resizable=yes");
...
function abrirRelatorioFaturamento(){
	...
	window.open('relatorios.php?cadastro=faturamento&filtro1=' + filtro1 + '&filtro2=' + filtro2 + '&filtro3=' + filtro3, "janelaRelatorios", "height=600, width=800, scrollbars=yes, resizable=yes");	
...
function abrirRelatorioVendas(){
	...
	window.open('relatorios.php?cadastro=vendas&filtro1=' + filtro1 + '&filtro2=' + filtro2 + '&filtro3=' + filtro3 + '&filtro4=' + filtro4, "janelaRelatorios", "height=600, width=800, scrollbars=yes, resizable=yes");
...
function abrirRelatorioEtiquetas(){
	...
	window.open('rel_etiquetas_gerar.php', "janelaRelatorios", "height=600, width=800, scrollbars=yes, resizable=yes");
...
function abrirRelatorioFluxo(){
	...
	window.open('relatorios.php?cadastro=fluxo', "janelaRelatorios", "height=600, width=800, scrollbars=yes, resizable=yes");
...
function excluirPedido(id){
	...
	doAjaxSemRetorno('ajax_com/pedidos_acao.php?acao=deletar&id=' + id, 1 , 'SaidaMain');
...
function excluirItemPedido(id, idPedido){
	...
	if(confirm('Deseja deletar item do pedido?')){
		...
		doAjaxSemRetorno('ajax_com/pedidos_acao.php?acao=deletarItemPedido&id=' + id, 1 , '');
...
function adicionarItemPedido(id, itemVerificar){
	...
	if(verificaPrecoEspecifico(itemVerificar)){
		...
		doAjaxSemRetorno('ajax_com/pedidos_acao.php?acao=adicionarItem&id=' + id, 1 , '');

		doAjaxSemRetorno('ajax_com/pedidos.php?acao=listarItens&idPedido=' + id, 1,'bodyID');
...
function selecionaProdutoItenPedido(id, indice, idItem){
	...
	doAjaxSemRetorno('ajax_com/pedidos.php?acao=selecionaProduto&idProduto=' + id + '&indice=' + indice + '&idItem=' + idItem, 1 , '');
...
function salvaCampo(campo, valor, idItem, indice){
	...
	doAjaxSemRetorno('ajax_com/pedidos_acao.php?acao=salvarItem&indice=' + indice + '&campo=' + campo + '&valor=' + valor + '&idItem=' + idItem, 1 , '');
...
function calculaPrecoPedido(  ){
	...
	for(var i=0; i<contI; i++){
		...
		$('campoTotal_'+i).innerHTML = $('qtd'+i).value * parseFloat(precoIten);
		...
		if($('tipoComissao1').checked == true){
			...
			$('campoTotal_'+i).innerHTML = ($('qtd'+i).value * $('preco'+i).value) - ( ( ($('preco'+i).value * v ) / 100) * $('qtd'+i).value );

			somaPreco += parseFloat($('campoTotal_'+i).innerHTML);

		}else{

			$('campoTotal_'+i).innerHTML = ($('qtd'+i).value * $('preco'+i).value) - (v * $('qtd'+i).value );
...
function salvaPedido(idPedido, status){
	...
	if(confirm('Deseja salvar o Pedido?')){
		...
		if((status == 4 || status == 5) && !confirm('Pedido fechado, certeza que deseja alterar?')){
	 	...
	 	if(formm.verificaF(this.id,'erroForm','erro')){
			...
			if(verificaPreco()){
				doAjaxSemRetorno('ajax_com/pedidos_acao.php?acao=salvarPedido&tipo_entrega=' + $('tipoentrega').value + '&obs=' + $('obs').value + '&codigo=' + $('codigo').value + '&status=' + $('status').value + '&clientes=' + $('clientes').value + '&representantes=' + $('representantes').value  + '&idPedido=' + idPedido + '&formaPagamento=' + $('formapagamento').value + '&imposto=' + $('imposto').value + '&valorEntrega=' + $('valorEntrega').value + '&comissao=' + $('comissao').value + '&dataimposto=' + $('dataimposto').value, 1 , 'bodyID');

				doAjaxSemRetorno('ajax_com/pedidos.php?acao=listarItens&idPedido=' + idPedido, 1,'bodyID');

				doAjaxSemRetorno('ajax_com/pedidos.php?acao=listar&clientes=' + $('clientesBusca').value + '&status=' + $('statusBusca').value,1,'SaidaMain');
...
function pegaRepresentantePedido( cliente ){
	...
	doAjax('ajax_com/pedidos_acao.php?acao=pegarRepresentanteCliente&cliente=' + cliente, 'retornoPegaRepresentantePedido', 0, '');
...
function pegaDadosClientePedidoTime( cliente ){
	...
	doAjax('ajax_com/pedidos_acao.php?acao=pegarDadosClientes&cliente=' + cliente, 'retornoPegaDadosClientePedido', 0, '');	
...
function enviarEmailPedido(idPedido, idCliente){
	...
	window.open('enviarEmail.php?tipo=fluxo&id=' + idPedido + '&idCliente=' + idCliente, 'Email', 'height = 200, width = 300, location = no, toolbar = no, menubar=no')
...
function impressaoPedido(idPedido){
	...
	window.open('impressao.php?cadastro=pedido&id=' + idPedido, 'Email',  "height=600, width=850, scrollbars=yes, resizable=yes")
...
function refreshPedido(){
	...
	doAjaxSemRetorno('ajax_com/pedidos.php?acao=listar&clientes=' + $('clientesBusca').value + '&status=' + $('statusBusca').value + '&dataIni=' + $('dataIni').value + '&dataFim=' + $('dataFim').value + '&codigo=' + $('codigoBusca').value + '&dataEnvioIni=' + $('dataEnvioIni').value + '&dataEnvioFim=' + $('dataEnvioFim').value,1,'SaidaMain');
...
function enviarEmail(id, tipo, email){
	...
	doAjaxSemRetorno('ajax_com/enviarEmail.php?id=' + id + '&tipo=' + tipo +  '&email=' + email,1,'Saida');
...
function refreshEstoque(){
	...
	doAjaxSemRetorno('ajax_com/estoque.php?acao=listar&produto=' + $('produtoBusca').value + '&tipo=' + $('tipoBusca').value + '&dataIni=' + $('dataIni').value + '&dataFim=' + $('dataFim').value,1,'Saida');
...
function salvaEstoque(){
	...
	if(confirm('Deseja salvar?')){
		...
		if(formm.verificaF(this.id,'erroForm','erro')){
			...
			doAjaxSemRetorno('ajax_com/estoque_acao.php?acao=salvar&descricao=' + $('descricao').value + '&produto=' + $('produto').value + '&id=' + $('id').value + '&tipo=' + $('tipo').value + '&qtd=' + $('qtd').value  + '&preco=' + $('preco').value + '&data=' + $('data').value, 1 , '');
...
function refreshOrdemProducao(){
	...
	doAjaxSemRetorno('ajax_com/ordem_producao.php?acao=listar&produto=' + $('produtoBusca').value + '&pedido=' + $('pedidoBusca').value + '&dataIni=' + $('dataIni').value + '&dataFim=' + $('dataFim').value,1,'Saida');
...
function refreshOrdemSeparacao(){
	...
	doAjaxSemRetorno('ajax_com/ordem_separacao.php?acao=listar&produto=' + $('produtoBusca').value + '&pedido=' + $('pedidoBusca').value + '&dataIni=' + $('dataIni').value + '&dataFim=' + $('dataFim').value,1,'Saida');
...
function excluirOrdemSeparacao(id, status){
	...
	if(status != 2){
		...
		doAjaxSemRetorno('ajax_com/ordem_separacao_acao.php?acao=deletar&id=' + id, 1 , '');
...
function refreshFluxo(){
	...
	doAjaxSemRetorno('ajax_com/fluxo.php?acao=listar&cliente=' + $('clienteBusca').value + '&tipo=' + $('tipoBusca').value + '&tipoFluxo=' + $('tipoFluxoBusca').value + '&dataIni=' + $('dataIni').value, 1, 'Saida');

}

function salvarFluxo(idFluxo){

	if(confirm('Deseja salvar?')){

		if(formm.verificaF(this.id,'erroForm','erro')){

			doAjaxSemRetorno('ajax_com/fluxo_acao.php?acao=salvar&cliente=' + $('clientes').value + '&id=' + $('id').value + '&fornecedor=' + $('fornecedor').value + '&tipo=' + $('tipo').value + '&tipoFluxo=' + $('tipoFluxo').value + '&ocorrencia=' + $('ocorrencia').value  + '&valor=' + $('valor').value + '&data=' + $('data').value + '&statusFluxo=' + $('statusFluxo').value, 1 , '');
...
function refreshFluxoBanco(){
	...
	doAjaxSemRetorno('ajax_com/fluxoBanco.php?acao=listarBancos&tipo=' + $('tipoBusca').value + '&tipoFluxo=' + $('tipoFluxoBusca').value + '&dataIni=' + $('dataIni').value , 1, 'Saida');

}

function refreshFluxoBancoFluxo(idBanco){

	doAjaxSemRetorno('ajax_com/fluxoBanco.php?acao=listar&idBanco=' + idBanco + '&tipo=' + $('tipoBusca').value + '&tipoFluxo=' + $('tipoFluxoBusca').value + '&dataIni=' + $('dataIni').value, 1, 'Saida');
...
function salvarFluxoBanco(idFluxo){
	...
	if(confirm('Deseja salvar?')){
		...
		if(formm.verificaF(this.id,'erroForm','erro')){
			...
			doAjaxSemRetorno('ajax_com/fluxoBanco_acao.php?acao=salvar&id=' + $('id').value + '&idBanco=' + $('banco').value + '&tipo=' + $('tipo').value + '&tipoFluxo=' + $('tipoFluxo').value + '&ocorrencia=' + $('ocorrencia').value  + '&valor=' + $('valor').value + '&data=' + $('data').value + '&numeroDoc=' + $('numero_doc').value, 1 , '');
...
function excluirContasReceber(id){

	doAjaxSemRetorno('ajax_com/contas_receber_acao.php?acao=cancelar&id=' + id, 1 , '');
...
function refreshComposicao(id){
	...
	doAjaxSemRetorno('ajax_com/composicao.php?acao=listar&id=' + id + '&itensBusca=' + $('itensBusca').value , 1, 'Saida'); 
...
function salvaComposicao(){
	...
	if(confirm('Deseja salvar?')){
		...
		if(formm.verificaF(this.id,'erroForm','erro')){
			...
			doAjaxSemRetorno('ajax_com/composicao_acao.php?acao=salvar&descricao=' + $('descricao').value + '&produto=' + $('produto').value + '&id=' + $('id').value + '&qtd=' + $('qtd').value + '&idComposicao=' + $('idComposicao').value, 1 , '');
...
function excluirCompraCotacao(id){
	...
	doAjaxSemRetorno('ajax_com/compras_acao.php?acao=deletar&id=' + id, 1 , 'SaidaMain');
...
function excluirItemCompra(id, idCompra){
	...
	if(confirm('Deseja deletar item?')){
		...
		doAjaxSemRetorno('ajax_com/compras_acao.php?acao=deletarItemCompra&id=' + id, 1 , '');
...
function salvaCampoCompra(campo, valor, idItem, indice){
	...
	doAjaxSemRetorno('ajax_com/compras_acao.php?acao=salvarItem&indice=' + indice + '&campo=' + campo + '&valor=' + valor + '&idItem=' + idItem, 1 , '');

}

function salvaCampoCompraFormaPgto(campo, valor, idItem, indice){

	doAjaxSemRetorno('ajax_com/compras_acao.php?acao=salvarFormaPgto&indice=' + indice + '&campo=' + campo + '&valor=' + valor + '&idItem=' + idItem, 1 , '');
...
function calculaPrecoCompra(  ){
	...
	$('precoTotalItens').innerHTML = parseFloat(somaPreco) + parseFloat(precoImposto) - parseFloat(precoDesc);
	...
	if(confirm('Deseja salvar ?')){
		...
		if((status == 1) && !confirm('Est� fechado, certeza que deseja alterar?')){
	 	...
	 	if(formm.verificaF(this.id,'erroForm','erro')){
                    ...
                    doAjaxSemRetorno('ajax_com/compras_acao.php?acao=salvarCompra&tipo_entrega=&obs=' + $('obs').value + '&codigo=' + $('codigo').value + '&fornecedores=' + $('fornecedores').value + '&idCompra=' + idCompra + '&imposto=' + $('imposto').value + '&desconto=' + $('desconto').value + '&tipoFluxo=' + $('tipoFluxo').value, 1 , 'bodyID');

                    doAjaxSemRetorno('ajax_com/compras.php?acao=listarItens&idCompra=' + idCompra, 1,'bodyID');

                    if(local == "cotacao"){
                        doAjaxSemRetorno('ajax_com/compras.php?acao=listarCotacao&fornecedores=' + $('fornecedoresBusca').value, 1, 'SaidaMain');
                    }else{
                        doAjaxSemRetorno('ajax_com/compras.php?acao=listarCompras&fornecedores=' + $('fornecedoresBusca').value, 1, 'SaidaMain');
                    }
...
function impressaoCompra(idCompra, tipo){
	...
	window.open('impressao.php?cadastro=compra&id=' + idCompra + '&tipo=' + tipo, 'Email',  "height=600, width=850, scrollbars=yes, resizable=yes")
...
function refreshCotacao(){
	...
	doAjaxSemRetorno('ajax_com/compras.php?acao=listarCotacao&fornecedores=' + $('fornecedoresBusca').value + '&dataIni=' + $('dataIni').value + '&dataFim=' + $('dataFim').value + '&codigo=' + $('codigoBusca').value, 1, 'SaidaMain');

}

function refreshCompras(){

	doAjaxSemRetorno('ajax_com/compras.php?acao=listarCompras&fornecedores=' + $('fornecedoresBusca').value + '&dataIni=' + $('dataIni').value + '&dataFim=' + $('dataFim').value + '&codigo=' + $('codigoBusca').value, 1, 'SaidaMain');

}

function adicionarItemCompra(id, itemVerificar){

    doAjaxSemRetorno('ajax_com/compras_acao.php?acao=adicionarItem&id=' + id, 1 , '');

    doAjaxSemRetorno('ajax_com/compras.php?acao=listarItens&idCompra=' + id, 1,'bodyID');

}

function adicionarFormaPgtoCompra(id, itemVerificar){

    doAjaxSemRetorno('ajax_com/compras_acao.php?acao=adicionarFormaPgto&id=' + id, 1 , '');
...
function abrirRelatorioFechamento(){
    main.openWindow('relatorios_fechamento.php?filtro1=' + $('filtro1').value + '&filtro2=' + $('filtro2').value,'500','600');
}

function abrirRelatorioDespesas(){
    main.openWindow('relatorios_despesas.php?filtro1=' + $('filtro1').value + '&filtro2=' + $('filtro2').value,'500','600');
}
...
function makeAnimationMenu(){
	...
	jQuery(".sidebar-collapse").mouseenter(function() {
			jQuery(".menu-submenu-nav").animate({ 'width':"295px"}, 100, function(){
				
			});	
	});
	
	jQuery(".navbar-default").mouseleave(function() {
		jQuery(".menu-submenu-nav").animate({'width':"0px"}, 200, function(){ 
		});
	});
